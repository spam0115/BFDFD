...
Public Class FileListDataSourceT
    Implements BrightIdeasSoftware.IVirtualListDataSource

    Private _dataList As TreeSet(Of cSimpleFile)

    Public Sub New(data As TreeSet(Of cSimpleFile))
        _dataList = data
    End Sub
    ...
    Public Sub Sort(column As BrightIdeasSoftware.OLVColumn, order As SortOrder) Implements BrightIdeasSoftware.IVirtualListDataSource.Sort
        If _dataList IsNot Nothing Then
            Select Case column.AspectName
                ...
                Case "_GroupId"
                    If order = SortOrder.Ascending Then
                        _dataList.Sort(Function(x, y) x.GroupId.CompareTo(y.GroupId))
                    Else
                        _dataList.Sort(Function(x, y) y.GroupId.CompareTo(x.GroupId))
                    End If
            End Select
        End If
    End Sub
    ...
    Public Sub AddObjects(modelObjects As ICollection) Implements IVirtualListDataSource.AddObjects
        If _dataList Is Nothing Then
            _dataList = New TreeSet(Of cSimpleFile)()
        End If
