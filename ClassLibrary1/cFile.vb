...
Public Class cFile
    ...
    ' ========================================
#Region "Attributes"
    Private _tFileVersion As FileVersionInfo
...
#End Region
    ...
    Public Sub Refresh()
        ...
        ' Infos about dll/exe
        _tFileVersion = FileVersionInfo.GetVersionInfo(_Path)
    ...
    Public Shared Function GetFileExtension(ByVal path As String) As String
        If path IsNot Nothing Then
            ...
            If x > 0 AndAlso x > InStrRev(path, "\", , CompareMethod.Binary) Then
                Return Right(path, path.Length - x)
            Else
                Return ""
            End If
        Else
    ...
    ' Return true if extension of the file belong to the list of extensions
    ' Extensions must be separated by ';' char
    Public Shared Function DoesExtensionMatch(ByVal file As String, ByVal exts As List(Of String)) As Boolean
        Dim ext As String = "*." & GetFileExtension(file).ToUpper
        For Each tt As String In exts
            If ext = tt.ToUpper OrElse tt = "*.*" Then
                Return True
            End If
        Next
        Return False
    ...
    Public Shared Function MoveFilesToTrash(ByVal files As List(Of String), ByVal hwnd As IntPtr) As Integer
        Dim s As New Native.Api.NativeStructs.SHFILEOPSTRUCT
        With s
            .fAnyOperationsAborted = 0
            .fFlags = Native.Api.NativeEnums.FILEOP_FLAGS_ENUM.FOF_ALLOWUNDO Or _
                    Native.Api.NativeEnums.FILEOP_FLAGS_ENUM.FOF_SIMPLEPROGRESS
            .hNameMappings = IntPtr.Zero
            .hwnd = hwnd
            .lpszProgressTitle = "Move files to trash"
            .pFrom = Marshal.StringToHGlobalUni(StringArrayToMultiString(files))
            .pTo = Marshal.StringToHGlobalUni(StringArrayToMultiString(Nothing))
            .wFunc = Native.Api.NativeEnums.FO_Func.FO_DELETE
        End With

        Dim ret As Integer = Native.Api.NativeFunctions.SHFileOperation(s)

        Native.Api.NativeFunctions.SHChangeNotify(Native.Api.NativeEnums.HChangeNotifyEventID.SHCNE_ALLEVENTS, _
                                Native.Api.NativeEnums.HChangeNotifyFlags.SHCNF_DWORD, _
                                IntPtr.Zero, _
                                IntPtr.Zero)
    ...
    ' Kill files
    Public Shared Function KillFiles(ByVal files As List(Of String), ByVal hwnd As IntPtr) As Integer
        Dim s As New Native.Api.NativeStructs.SHFILEOPSTRUCT
        With s
            .fAnyOperationsAborted = 0
            .fFlags = Native.Api.NativeEnums.FILEOP_FLAGS_ENUM.FOF_SIMPLEPROGRESS
            .hNameMappings = IntPtr.Zero
            .hwnd = hwnd
    ...
    ' Move files
    Public Shared Function MoveFiles(ByVal files As List(Of String), ByVal dest As String, ByVal hwnd As IntPtr) As Integer
        Dim s As New Native.Api.NativeStructs.SHFILEOPSTRUCT
        Dim toDir As New List(Of String)
        toDir.Add(dest)
        With s
            .fAnyOperationsAborted = 0
            .fFlags = Native.Api.NativeEnums.FILEOP_FLAGS_ENUM.FOF_SIMPLEPROGRESS
            .hNameMappings = IntPtr.Zero
            .hwnd = hwnd
            .lpszProgressTitle = "Move files"
            .pFrom = Marshal.StringToHGlobalUni(StringArrayToMultiString(files))
            .pTo = Marshal.StringToHGlobalUni(StringArrayToMultiString(toDir))
            .wFunc = Native.Api.NativeEnums.FO_Func.FO_MOVE
        End With
    ...
    Private Shared Function StringArrayToMultiString(ByVal strings As List(Of String)) As String
